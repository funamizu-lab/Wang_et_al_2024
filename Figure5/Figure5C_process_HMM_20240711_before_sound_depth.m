...
----------------------------------------------------------------------------
%}
function Figure5C_process_HMM_20240711_before_sound_depth

% repeat
HMM_20240711_CurChoiceIndex_before_sound_depth('repeat_OFC_20230427',1);
HMM_20240711_CurChoiceIndex_before_sound_depth('repeat_AC_20230427',1);


% % zigzag
HMM_20240711_CurChoiceIndex_before_sound_depth('zigzag_OFC_20230427',1);
HMM_20240711_CurChoiceIndex_before_sound_depth('zigzag_AC_20230427',1);
HMM_20240711_CurChoiceIndex_before_sound_depth('zigzag_PPC_20230427',1);
HMM_20240711_CurChoiceIndex_before_sound_depth('zigzag_M1_20230427',1);
HMM_20240711_CurChoiceIndex_before_sound_depth('zigzag_STR_20230427',1);


return

function HMM_20240711_CurChoiceIndex_before_sound_depth(folders, kaiseki_number)
...
all_pre_same_pLcL_pLcH_index =[];
all_p_pre_same_pLcL_pLcH_index =[];
all_pre_same_pHcL_pHcH_index =[];
all_p_pre_same_pHcL_pHcH_index =[];
all_cur_same_pLcL_pHcL_index =[];
all_p_cur_same_pLcL_pHcL_index =[];
all_cur_same_pLcH_pHcH_index =[];
all_p_cur_same_pLcH_pHcH_index =[];
...
for i = 1:length(analysis_dir)
    ...
    all_pre_same_pLcL_pLcH_index =[all_pre_same_pLcL_pLcH_index;pre_same_pLcL_pLcH_index];
    all_pre_same_pHcL_pHcH_index =[all_pre_same_pHcL_pHcH_index;pre_same_pHcL_pHcH_index];    
    all_cur_same_pLcL_pHcL_index =[all_cur_same_pLcL_pHcL_index;cur_same_pLcL_pHcL_index];    
    all_cur_same_pLcH_pHcH_index =[all_cur_same_pLcH_pHcH_index;cur_same_pLcH_pHcH_index];

    all_p_pre_same_pLcL_pLcH_index =[all_p_pre_same_pLcL_pLcH_index;p_pre_same_pLcL_pLcH_index];
    all_p_pre_same_pHcL_pHcH_index =[all_p_pre_same_pHcL_pHcH_index;p_pre_same_pHcL_pHcH_index];
    all_p_cur_same_pLcL_pHcL_index =[all_p_cur_same_pLcL_pHcL_index;p_cur_same_pLcL_pHcL_index];
    all_p_cur_same_pLcH_pHcH_index =[all_p_cur_same_pLcH_pHcH_index;p_cur_same_pLcH_pHcH_index];
...
if contains(brainarea,'repeat')
    ...
    sig_sound = find(all_p_pre_sound(:,3) < 0.01); 
else 
    left_neuron = find(all_pre_choice_correct_error(:,1) >= 0);
    right_neuron = find(all_pre_choice_correct_error(:,1) < 0);
    sig_sound = find(all_p_pre_sound(:,3) < 0.01);
...
abs_sig_all_pre_same_pLcL_pLcH_index = abs(all_pre_same_pLcL_pLcH_index);
abs_sig_all_pre_same_pHcL_pHcH_index = abs(all_pre_same_pHcL_pHcH_index);
abs_sig_all_cur_same_pLcL_pHcL_index = abs(all_cur_same_pLcL_pHcL_index);
abs_sig_all_cur_same_pLcH_pHcH_index = abs(all_cur_same_pLcH_pHcH_index);
% 
ave_pre_same = mean([abs_sig_all_pre_same_pLcL_pLcH_index, abs_sig_all_pre_same_pHcL_pHcH_index],2);
ave_cur_same = mean([abs_sig_all_cur_same_pLcL_pHcL_index, abs_sig_all_cur_same_pLcH_pHcH_index],2);
p_index = signrank(ave_pre_same,ave_cur_same);


figure
plot(ave_pre_same(non_sig_neuron), ave_cur_same(non_sig_neuron), 'k.') %same choice, X_repeat or Y_zigzag
hold on
plot(ave_pre_same(left_sig_neuron), ave_cur_same(left_sig_neuron), 'b.') %same choice, X_repeat or Y_zigzag
hold on
plot(ave_pre_same(right_sig_neuron), ave_cur_same(right_sig_neuron), 'r.') %same choice, X_repeat or Y_zigzag
hold on
plot([-0.1, 1],[-0.1, 1],'k')
text(0.6,0,['p = ' string(p_index)])
set(gca,'xlim',[-0.1, 1],'ylim',[-0.1, 1],'fontname','Arial','Box','off')
